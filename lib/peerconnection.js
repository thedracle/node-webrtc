...
function RTCPeerConnection(configuration, constraints) {
  ...
  var that = this
    ...
    , pending = null
    , dataChannels = {}
    , mediaStreams = {};  // open data channels, indexed by label
...
//    }
  ...
  pc.onicecandidate = function onicecandidate(candidate, sdpMid, sdpMLineIndex) {
    ...
    that.dispatchEvent(iceCandidateEvent);
    that._onicecandidate && that._onicecandidate(iceCandidateEvent);
  };
  ...
  pc.onsignalingstatechange = function onsignalingstatechange(state) {
    ...
    that._onsignalingstatechange && that._onsignalingstatechange({type: 'signalingstatechange'});
  };

  pc.oniceconnectionstatechange = function oniceconnectionstatechange(state) {
    that.dispatchEvent({type: 'iceconnectionstatechange'});
    that._oniceconnectionstatechange && that._oniceconnectionstatechange({type: 'iceconnectionstatechange'});
  };

  pc.onicegatheringstatechange = function onicegatheringstatechange(state) {
    that.dispatchEvent({type: 'icegatheringstatechange'});

    that._onicegatheringstatechange && that._onicegatheringstatechange({type: 'icegatheringstatechange'});
  ...
  pc.ondatachannel = function ondatachannel(internalDC) {
      ...
      that.dispatchEvent(dcEvent);
      that._ondatachannelcb && that._ondatachannelcb(dcEvent);
  }
  pc.onaddstream = function(internalMS) {
    mediaStreams[internalMS.id] = internalMS;

    var ms = new RTCMediaStream(internalMS);

    var addStreamEvent = new RTCMediaStreamEvent('mediastream', {stream: ms})
    that.dispatchEvent(addStreamEvent);
    that._onaddstream && that._onaddstream(addStreamEvent);

  };

  pc.onvideoframe = function(label, width, height, yPlane, uPlane, vPlane) {
    var videoFrameEvent = new RTCVideoFrameEvent('videoframe',
        {label: label, width: width, height: height, yPlane: yPlane, uPlane: uPlane, vPlane: vPlane});
    that.dispatchEvent(videoFrameEvent);
    that._onvideoframe && that._onvideoframe(videoFrameEvent);
  };

  pc.onencodedvideoframe = function(label, width, height, encodedFrame) {
      var videoFrameEvent = new RTCVideoFrameEvent('videoframe',
          {label: label, width: width, height: height, encodedFrame: encodedFrame});
      that.dispatchEvent(videoFrameEvent);
      that._onencodedvideoframe && that._onencodedvideoframe(videoFrameEvent);
  };

  pc.onnegotiationneeded = function() {
    that.dispatchEvent({type: 'negotiationneeded'});
    that._onnegotiationneeded && that._onnegotiationneeded();
  }
  ...
  Object.defineProperties(this, {
    'ondatachannel': {
      set: function(onDC) {
        that._ondatachannelcb = onDC;
      }
    },
    'onaddstream': {
      set: function(onAddStream) {
        that._onaddstream = onAddStream;
      }
    },
    'onicegatheringstatechange' : {
      set: function(onIceGatheringStateChange) {
        this._onicegatheringstatechange = onIceGatheringStateChange;
      }
    },
    'oniceconnectionstatechange': {
      set: function(onIceConnectionStateChange) {
        this._oniceconnectionstatechange = onIceConnectionStateChange;
      }
    },
    'onsignalingstatechange': {
      set: function(onSignalingStateChange) {
        this._onsignalingstatechange = onSignalingStateChange;
      }
    },
    'onicecandidate': {
    ...
    'onvideoframe': {
      set: function(onVideoFrame) {
        this._onvideoframe = onVideoFrame;
      }
    },
    'onencodedvideoframe': {
        set: function(onEncodedVideoFrame) {
            this._onencodedvideoframe = onEncodedVideoFrame;
        }
    },
    'onnegotiationneeded': {
